...
function showSolveDetails(index, time, scramble) {
  displayContent(index, time, scramble)
  showModal.value = true
}
function displayContent(index, time, scramble) {
  let newContent = '<div class="font-bold text-2xl my-3"> Time [' + index + '] </div>'
  newContent += '<div class="font-bold text-5xl my-2">' + displayTime(time) + '</div>'
  newContent += '<div class="font-semibold text-xl">' + scramble + '</div>'
